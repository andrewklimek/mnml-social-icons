...
function socialmonger( $a, $c ) {
	...
	$out = "<ul id='socialmonger-{$idno}' class='socialmonger'>";
	...
	if ( $idno === 1 ) {
		...
		$out .= "
		<style>
		.socialmonger {display: inline-table;padding: 0;}
		.socialmonger a {text-decoration: none;}
		.socialmonger li {display: table-cell;vertical-align: middle; padding: {$padding};}
		.socialmonger svg {display: block;width: {$size};height: {$size};fill: {$color};stroke: {$color};stroke-width: {$outline};}
		</style>";
		...
		if ( !empty( $a['padding'] ) )
			$out .= "} #socialmonger-{$idno} li {padding: {$a['padding']};";
		$out .= "}</style>";
